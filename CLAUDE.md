...
- **User metadata**: `x-amz-meta-*` is persisted in the object's `.meta.json` sidecar (names lowercased, prefix stripped) and returned on GET/HEAD. Capped at 2 KB total (`MetadataTooLarge` above that) and validated on write, since a value `http` refuses would panic the response builder on every read. Immutable after write, so it is bound into the SSE sidecar MAC (unlike tags). `CopyObject` honours `x-amz-metadata-directive`
